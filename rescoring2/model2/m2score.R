...
tst=args[3]
w=as.numeric(args[4])
d=read.csv(sprintf("../pdbbind-%s-trn-%s-yxi.csv",v,trn)) # d[1] is pbindaff in pKd unit, d[2] to d[6] are Vina"s 5 unweighted terms, d[7] and d[8] are N(ActTors) and N(InactTors).
...
d=read.csv(sprintf("../tst-%s-yxi.csv",tst)) # d[1] is pbindaff in pKd unit, d[2] to d[6] are Vina"s 5 unweighted terms, d[7] and d[8] are N(ActTors) and N(InactTors).
d["t"]=1+w*(d[7]+0.5*d[8]) # Flexibility penalty = 1 + w * Nrot. In Vina"s implementation, active torsions are counted as 1, while inactive torsions (i.e. -OH,-NH2,-CH3) are counted as 0.5.
d["p"]=(c[1]+d[2]*c[2]+d[3]*c[3]+d[4]*c[4]+d[5]*c[5]+d[6]*c[6])/d["t"] # Binding affinity predicted by the newly-trained model, with kcal/mol converted to pKd.
