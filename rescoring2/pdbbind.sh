...
#		ln -s ../${c}_ligand.pdbqt out/${c}_ligand_ligand_0.pdbqt
		...
		tail -n +26 log/${c}_ligand.txt | head -$n | awk -v y=$(cat pbindaff.txt) 'function abs(x){return x < 0 ? -x : x} {print abs(substr($0,13,5) * -0.73349480509 - y)}' | paste ../../seq$n - | sort -s -k2,2n | head -1 | cut -f1 > vina-scheme-4.txt
		for i in {1..9}; do
