...
d=read.csv('../pdbbind-2007-refined-core-yxi.csv') # d[1] is pbindaff in pKd unit, d[2] to d[6] are Vina's 5 unweighted terms, d[7] and d[8] are N(ActTors) and N(InactTors).
...
#write.csv(c(d['PDB'],round(d['pbindaff'],2),round(d['p'],2)),row.names=F,quote=F,file="pdbbind-2007-refined-core-iyp.csv") # Write the measured binding affinities and the predicted ones in CSV format.
...
cat(sprintf("n,rmse,sdev,pcor,scor,kcor\n%d,%.3f,%.3f,%.3f,%.3f,%.3f\n", n, rmse, sdev, pcor, scor, kcor), file="pdbbind-2007-refined-core-statistics.csv")
...
scor=cor(d['p'], d[1], method="spearman")
kcor=cor(d['p'], d[1], method="kendall")
cat(sprintf("n,rmse,sdev,pcor,scor,kcor\n%d,%.3f,%.3f,%.3f,%.3f,%.3f\n", n, rmse, sdev, pcor, scor, kcor), file="pdbbind-2007-core-statistics.csv")
