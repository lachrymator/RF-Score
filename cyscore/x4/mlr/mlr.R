...
for (ntst in c(195,201,382))
{
	...
	write.csv(c(d["PDB"],d["pbindaff"],d["predicted"],d["regressed"]),row.names=F,quote=F,file=sprintf("tst-%s-iypr.csv",ntst))
	n=nrow(d)
	...
	cat(sprintf("n,rmse,sdev,pcor,scor,kcor\n%d,%.3f,%.3f,%.3f,%.3f,%.3f\n", n, rmse, sdev, pcor, scor, kcor),file=sprintf("tst-%s-stat.csv",ntst))
}
